...
sft_path = os.path.join(model_path,"sft.pth")
dpo_path = os.path.join(model_path,"dpo.pth")


tinylm = gpt0.TextGpt(d_model=512,num_attention_heads=16,num_layers=8,context_len=1024)
...
if os.path.exists(pretrain_path):
	...
	for dt in pt():
		...
		loss = tinylm.learn(dt, epochs=1, learning_rate=1e-4, batch_size=10)
		cur_time = time.time()
		if cur_time - last_time > 1:
			print("预训练总样本数：",data_count,",字节数：",total_bytes,",损失值：%.5f"%loss,",用时：",int(time.time() - time_start),",每秒字节数：%d"%(bytes_per_second/(cur_time - last_time)),",每秒样本数：%d"%(sample_per_second/(cur_time - last_time)),flush=True)
			sample_per_second = 0
			bytes_per_second = 0
			last_time = time.time()
...
if os.path.exists(sft_path):
	...
	for dt in sft():
		...
		cur_time = time.time()
		if cur_time - last_time > 1:
			print("微调总样本数：",data_count,",字节数：",total_bytes,",损失值：%.5f"%loss,",用时：",int(time.time() - time_start),",每秒字节数：%d"%(bytes_per_second/(cur_time - last_time)),",每秒样本数：%d"%(sample_per_second/(cur_time - last_time)),flush=True)
			sample_per_second = 0
			bytes_per_second = 0
			last_time = time.time()
	tinylm.save_model(sft_path)
...
if os.path.exists(dpo_path):
	...
	for dt in dbo():
		...
		bytes_per_second += batch_bytes
		sample_per_second += len(dt)
		loss = tinylm.learn(dt, epochs=1, learning_rate=
		1e-4, batch_size=10)
		cur_time = time.time()
		if cur_time - last_time > 1:
			print("对抗训练总样本数：",data_count,",字节数：",total_bytes,",损失值：%.5f"%loss,",用时：",int(time.time() - time_start),",每秒字节数：%d"%(bytes_per_second/(cur_time - last_time)),",每秒样本数：%d"%(sample_per_second/(cur_time - last_time)),flush=True)
			sample_per_second = 0
			bytes_per_second = 0
			last_time = time.time()
